    ...
    drawCanvas: (ctx) => {
        ...
        if (canvas) {
            ...
            activeTrades.forEach(trade => {
                    ...
                    const entryCandleIndexCalc = trade.entryTickIndex / state.ticksPerCandle;
                    const xEntry = getX(entryCandleIndexCalc);
                    const durationInSeconds = trade.duration / 1000;
                    const durationInTicks = durationInSeconds * conf.TICK_RATE;
                    const expireCandleIndexCalc = entryCandleIndexCalc + (durationInTicks / state.ticksPerCandle);
                    const xExpire = getX(expireCandleIndexCalc);
